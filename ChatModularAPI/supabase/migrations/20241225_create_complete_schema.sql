-- Migration: Create complete schema with all required tables
-- Date: 2024-12-25
...
-- Update existing chat tables to reference new structure
-- First, check if chat tables exist and update them
DO $$
BEGIN
    -- Update UsuariosChat if it exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'usuarioschat') THEN
        -- Add foreign key columns if they don't exist
        IF NOT EXISTS (SELECT FROM information_schema.columns WHERE table_name = 'usuarioschat' AND column_name = 'nUsuariosChatEmpresaId') THEN
            ALTER TABLE UsuariosChat ADD COLUMN nUsuariosChatEmpresaId UUID;
            ALTER TABLE UsuariosChat ADD COLUMN nUsuariosChatAplicacionId UUID;
            ALTER TABLE UsuariosChat ADD COLUMN cUsuariosChatPerCodigo VARCHAR(100);
            ALTER TABLE UsuariosChat ADD COLUMN cUsuariosChatPerJurCodigo VARCHAR(100);
        END IF;
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_Contactos_unique_pair ON Contactos(nContactosEmpresaId, nContactosAplicacionId, cContactosUsuarioSolicitanteId, cContactosUsuarioContactoId);

-- Create unique constraint to prevent duplicate contacts
CREATE UNIQUE INDEX IF NOT EXISTS idx_Contactos_unique_relationship ON Contactos(
    nContactosEmpresaId, 
    nContactosAplicacionId, 
    LEAST(cContactosUsuarioSolicitanteId, cContactosUsuarioContactoId),
    GREATEST(cContactosUsuarioSolicitanteId, cContactosUsuarioContactoId)
) WHERE bContactosActivo = true;
...
ALTER TABLE ConfiguracionEmpresa ENABLE ROW LEVEL SECURITY;
ALTER TABLE AppRegistro ENABLE ROW LEVEL SECURITY;
ALTER TABLE TokenRegistro ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for Usuarios
CREATE POLICY IF NOT EXISTS "Users can view users in their company" ON Usuarios FOR SELECT USING (
    nUsuariosEmpresaId IN (
        SELECT nUsuariosEmpresaId FROM Usuarios WHERE cUsuariosId = auth.uid()::text
    )
);

CREATE POLICY IF NOT EXISTS "Users can update their own data" ON Usuarios FOR UPDATE USING (
    cUsuariosId = auth.uid()::text
);

-- Create RLS policies for Contactos
CREATE POLICY IF NOT EXISTS "Users can view their contacts" ON Contactos FOR SELECT USING (
    cContactosUsuarioSolicitanteId = auth.uid()::text OR 
    cContactosUsuarioContactoId = auth.uid()::text
);

CREATE POLICY IF NOT EXISTS "Users can create contact requests" ON Contactos FOR INSERT WITH CHECK (
    cContactosUsuarioSolicitanteId = auth.uid()::text
);

CREATE POLICY IF NOT EXISTS "Users can update their contact requests" ON Contactos FOR UPDATE USING (
    cContactosUsuarioSolicitanteId = auth.uid()::text OR 
    cContactosUsuarioContactoId = auth.uid()::text
);
...
DECLARE
    v_contacto_id UUID;
    v_contacto_existente UUID;
BEGIN
    -- Verificar si ya existe un contacto entre estos usuarios
    SELECT nContactosId INTO v_contacto_existente
    FROM Contactos
    ...
    IF v_contacto_existente IS NOT NULL THEN
...
INSERT INTO ConfiguracionEmpresa (
    ...
    true,
    true
) ON CONFLICT (nConfiguracionEmpresaAplicacionId, nConfiguracionEmpresaEmpresaI